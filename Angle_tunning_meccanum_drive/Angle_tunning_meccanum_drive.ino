...
// Target
double TARGET_DISTANCE_X = 0;
double TARGET_DISTANCE_Y = 0.9; 
...
// ─────────────────────────────────────────────────────────────────────────────
double KP_INNER_VEL = 600.0; // <-- Softened from 700 to stop arrival vibrations
...
double KP_DIST = 2.5; 
double KP_ANGLE = 1.0; // Tuned via Serial (Key 'k')
...
void checkSerialTuner() {
    if (Serial.available() > 0) {
        ...
        if (key == 'k' || key == 'K') {
            ...
            if (val >= 0.0) {
                KP_ANGLE = val;
                Serial.printf(">>> UPDATED KP_ANGLE: %.2f\n", KP_ANGLE);
            }
        } else if (key == 't' || key == 'T') {
            double val = Serial.parseFloat();
            TARGET_ANGLE_DEG = val;
            Serial.printf(">>> UPDATED TARGET ANGLE: %.1f deg\n", TARGET_ANGLE_DEG);
        }
...
void setup() {
    ...
    Serial.println("--- TUNER ONLINE ---");
    Serial.println("Send 'k[value]' to alter KP_ANGLE (e.g. k1.5)");
    Serial.println("Send 't[value]' to alter target angle (e.g. t90.0)");
}
...
void loop() {
    ...
    if (now - last_inner_time >= INNER_LOOP_MS) {
        ...
        // 1. ROTATIONAL LOOP DRIVER (Calculated first to establish the velocity scale)
        if (abs(error_theta) < 0.04) { // <-- Widen deadband to 0.04 to stop target vibration
            target_robot_omega = 0.0;
        } else {
            target_robot_omega = error_theta * KP_ANGLE;
        }

        // 2. THE DYNAMIC GOVERNOR (Cross-Coupling Mechanism)
        // Normalizes the alignment error window down from 90 degrees (PI/2)
        double translation_scale = 1.0 - (abs(error_theta) / (PI / 2.0));
        translation_scale = constrain(translation_scale, 0.1, 1.0); // Keep tracking alive at minimum 10%

        // 3. LINEAR LOOP DRIVER (Now modulated by translation_scale)
        ...
        Serial.printf("Glob_X:%.2fm | Glob_Y:%.2fm | Ang:%.1f° | Scale:%.2f | K_Ang:%.2f | Cmd_W:%.2f\n", 
                      robot_pose_x, robot_pose_y, current_angle_deg, translation_scale, KP_ANGLE, target_robot_omega);
    } 
}
