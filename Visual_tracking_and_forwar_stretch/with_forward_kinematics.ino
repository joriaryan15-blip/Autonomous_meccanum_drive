...
// Heading tracker calculated from wheel kinematics (Odometry)
...
const double current_battery_voltage = 16.36;  //////////////////////////----Battery
...
void setup() {
    ...
    Serial.println("--- SYSTEM ARMED: WAYPOINT CONTROL SENDER ARMED (ODOMETRY ONLY) ---");
}
...
void loop() {
    ...
    if (current_micros - last_inner_time_micros >= (INNER_LOOP_MS * 1000)) {
        ...
        // ─────────────────────────────────────────────────────────────────
        // INTEGRATE TRACKED ANGULAR VELOCITY (W) TO COMPUTE ERROR HEADING
        // ─────────────────────────────────────────────────────────────────
        imu_yaw_rad += actual_robot_omega * dt;

        // Keep heading bound within [-PI, PI]
        while (imu_yaw_rad > PI)  imu_yaw_rad -= 2.0 * PI;
        while (imu_yaw_rad < -PI) imu_yaw_rad += 2.0 * PI;
        ...
        if (now_ms - last_log_time_ms >= 100) {
            ...
            if (sequence_step == 0) {
                Serial.printf("ST:0_WAY | X:%.2f->%.2f | Y:%.2f->%.2f | CalcHeading:%.2f | w:%.2f\n", robot_pose_x, TARGET_DISTANCE_X, robot_pose_y, TARGET_DISTANCE_Y, imu_yaw_rad, target_robot_omega);
            } else if (sequence_step == 1) {
                Serial.printf("ST:1_CAM | PixErrX:%.1f | X_Locked:%.2f | CalcHeading:%.2f | w:%.2f\n", udp_x - PIX_CENTER, robot_pose_x, imu_yaw_rad, target_robot_omega);
            } else if (sequence_step == 2) {
                Serial.printf("ST:2_STRCH | CurrX:%.2fm -> TargetX:%.2fm | CalcHeading:%.2f | w:%.2f\n", robot_pose_x, FINAL_TARGET_X, imu_yaw_rad, target_robot_omega);
            } else {
